...
export const EventMapCluster = memo(function EventMapCluster({
  ...
  useEffect(() => {
    if (!mapContainer.current || isLoaded || eventsWithCoords.length === 0) return;
    ...
    const initMap = async () => {
      try {
        ...
        if (!mapRef.current && mapContainer.current) {
          ...
          mapRef.current = L.map(mapContainer.current).fitBounds(bounds, {
            padding: [50, 50],
          });
        ...
        <div className="absolute bottom-4 left-4 right-4 z-1000 bg-zinc-900/95 backdrop-blur-sm rounded-xl border border-zinc-700 p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <Link
              ...
              className="shrink-0 flex items-center gap-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition"
            >
